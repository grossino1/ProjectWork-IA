...
RESET MANUALE:
  - Quando un episodio finisce, il programma si ferma
  - Tu premi "Continue" e "New Practice" in TORCS
  - Poi premi INVIO nel prompt per far partire l'episodio successivo
...
# ---------------------------------------------------------------------------
# TEST PRINCIPALE (MODIFICATO PER RESET MANUALE)
# ---------------------------------------------------------------------------
def test_hybrid(anchor_path: str, cork_path: str, n_episodes: int):
    ...
    for episode in range(1, n_episodes + 1):
        ...
        if episode > 1:
            print("\n⚠️  EPISODIO PRECEDENTE TERMINATO")
            print("\n📋 COSA FARE:")
            print("  1. In TORCS: clicca 'Continue'")
            print("  2. Seleziona 'New Practice'")
            print("  3. Attendi che la pista si carichi")
        ...
        while not connected:
            ...
            try:
                ...
                # Episodi successivi: relaunch=False (l'utente ha già fatto new practice)
        ...
        # ---- Esecuzione episodio (UGUALE A PRIMA) ----
        while not done:
