...
class Client():
    ...
    def setup_connection(self):
        ...
        while True:
            ...
            try:
                ...
                if n_fail < 0:
                    print("Connection timeout. Please ensure TORCS is in 'Race > Practice > New Race' mode.")
                    raise TimeoutError("Could not connect to TORCS server.")
                n_fail -= 1
    ...
    def get_servers_input(self):
        ...
        while True:
            ...
            if '***identified***' in sockdata:
                ...
                race_pos = self.S.d.get('racePos', 0)
                ...
                # Modificato: non chiudiamo più il socket, lasciamo che il simulatore si resetti
    ...
    def flush(self):
        """Svuota il buffer UDP per eliminare pacchetti obsoleti."""
        if not self.so: return
        self.so.settimeout(0.01)
        while True:
            try:
                self.so.recvfrom(data_size)
            except:
                break
