...
DATASET_PATH = os.path.join(BASE_DIR, "logs", f"manual_log_{timestamp_str}.jsonl")
...
def manual_recording():
    ...
    # Apertura file in modalità scrittura per il jsonl
    json_file = open(DATASET_PATH, "w", encoding="utf-8")
    step_count = 0
    t0 = time.time()
    initial_damage = None
        ...
        while True:
            try:
                ...
                if "***shutdown***" in sockstr:
                    print("\n>>> Il server ha interrotto la gara. Chiusura script in corso...")
                    break
            ...
            # --- LOGICA CAMBIO E RETROMARCIA ---
            # Se siamo (quasi) fermi o andiamo all'indietro e premiamo il freno
            if (speed < 1.0 and brake > 0.1) or speed < -1.0:
                target_gear = -1
                # Arcade mode: invertiamo freno e acceleratore. 
                # Il pedale del freno agisce da acceleratore in retromarcia.
                temp = brake
                brake = accel
                accel = temp
            else:
            ...
            # Mantieni la marcia in curva per stabilità (ma non bloccare la retro)
            ...
            # Scrittura Record in formato JSONL
            record = {
                "user": "manual_driver",
                "timestamp": time.time(),
                "sensors": {
                    "speedX": S.d.get("speedX", 0.0),
                    "speedY": S.d.get("speedY", 0.0),
                    "angle": S.d.get("angle", 0.0),
                    "trackPos": S.d.get("trackPos", 0.0),
                    "rpm": S.d.get("rpm", 0.0),
                    "track": S.d.get("track", [0.0]*19)
                },
                "actions": {
                    "steer": float(steer),
                    "accel": float(accel),
